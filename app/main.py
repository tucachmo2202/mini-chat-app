...
async def websocket_chat(
    websocket: WebSocket, room_id: str, token: str = Depends(get_current_user)
):
    await websocket.accept()
    await websocket_endpoint(websocket, broadcast, room_id, token)
...
async def send_message(
    ...
    redis.zadd(
        f"messages:{message.room_id}", {json.dumps(message_data.dict()): send_time}
    )
    await broadcast.publish(
        channel=message.room_id, message=json.dumps(message_data.dict())
    )
